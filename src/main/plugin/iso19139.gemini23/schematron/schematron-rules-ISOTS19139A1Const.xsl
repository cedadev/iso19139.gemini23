    ...
    If DQ_DataQuality.scope.level is 'dataset' or 'series' and no source or processStep is provided, then lineage statement is mandatory
      ...
      'maximumOccurrence' and 'domainValue' are mandatory
         ...
         <svrl:active-pattern>
            ...
            <xsl:attribute name="id">ISO19139A1_ROW19_InnerTextPattern_Obligation2</xsl:attribute>
            <xsl:attribute name="name">ISO19139A1_ROW19_InnerTextPattern_Obligation2</xsl:attribute>
            <xsl:apply-templates/>
         </svrl:active-pattern>
      ...
      Angle: The angle Unit of Measurement must be instantiated using the UomAngle_PropertyType
         ...
         <svrl:active-pattern>
            ...
            <xsl:attribute name="name">Element Values have no Nil Reason Attributes</xsl:attribute>
            <xsl:apply-templates/>
         </svrl:active-pattern>
...
<xsl:choose>
  ...
  <!--RULE
      -->
<xsl:template match="//gmd:MD_LegalConstraints/gmd:otherConstraints | //*[@gco:isoType='gmd:MD_LegalConstraints']/gmd:otherConstraints"
                 priority="1000"
                 mode="M16">
      <svrl:fired-rule xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                       context="//gmd:MD_LegalConstraints/gmd:otherConstraints | //*[@gco:isoType='gmd:MD_LegalConstraints']/gmd:otherConstraints"/>

      <!--ASSERT
      -->
<xsl:choose>
         <xsl:when test="string(normalize-space(./*[1]/text())) or         (./@gco:nilReason = 'inapplicable' or         ./@gco:nilReason = 'missing' or         ./@gco:nilReason = 'template' or         ./@gco:nilReason = 'unknown' or         ./@gco:nilReason = 'withheld')"/>
         <xsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl" ref="#_{geonet:element/@ref}"
                                test="string(normalize-space(./*[1]/text())) or (./@gco:nilReason = 'inapplicable' or ./@gco:nilReason = 'missing' or ./@gco:nilReason = 'template' or ./@gco:nilReason = 'unknown' or ./@gco:nilReason = 'withheld')">
               <xsl:attribute name="location">
                  <xsl:apply-templates select="." mode="schematron-select-full-path"/>
               </xsl:attribute>
               <svrl:text>
...
<xsl:choose>
   ...
   <!--PATTERN
        ISO19139A1_ROW19_InnerTextPattern_Obligation2-->


  <!--RULE
      -->
<xsl:template match="//gmd:MD_ExtendedElementInformation/gmd:obligation | //*[@gco:isoType = 'gmd:MD_ExtendedElementInformation']/gmd:obligation"
                 priority="1000"
                 mode="M28">
      <svrl:fired-rule xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                       context="//gmd:MD_ExtendedElementInformation/gmd:obligation | //*[@gco:isoType = 'gmd:MD_ExtendedElementInformation']/gmd:obligation"/>
...
<xsl:choose>
   ...
   <!--PATTERN
        Element Values have no Nil Reason Attributes-->
<svrl:text xmlns:svrl="http://purl.oclc.org/dsdl/svrl">Element Values have no Nil Reason Attributes</svrl:text>

  <!--RULE
      -->
<xsl:template match="//*[gco:CharacterString]" priority="1000" mode="M48">
      <svrl:fired-rule xmlns:svrl="http://purl.oclc.org/dsdl/svrl" context="//*[gco:CharacterString]"/>

      <!--ASSERT
      -->
<xsl:choose>
         <xsl:when test="not(string(normalize-space(gco:CharacterString))) or (string(normalize-space(gco:CharacterString)) and not(@gco:nilReason))"/>
         <xsl:otherwise>
            <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl" ref="#_{geonet:element/@ref}"
                                test="not(string(normalize-space(gco:CharacterString))) or (string(normalize-space(gco:CharacterString)) and not(@gco:nilReason))">
               <xsl:attribute name="location">
                  <xsl:apply-templates select="." mode="schematron-select-full-path"/>
               </xsl:attribute>
               <svrl:text>
      The '<xsl:text/>
                  <xsl:copy-of select="name()"/>
                  <xsl:text/>' element has a value, should not have gco:nilReason.
      </svrl:text>
            </svrl:failed-assert>
         </xsl:otherwise>
      </xsl:choose>
